...
/**
 * https://github.com/joffray589/steam-tradeoffer-manager-ts
 */
declare class TradeOfferManager {
  ...
  on(
    ...
    callback: (offer: TradeOfferManager.TradeOffer, reason: string) => any
  );
  on(
    event: "sentPendingOfferCanceled",
    callback: (offer: TradeOfferManager.TradeOffer) => any
  );
  on(
  ...
  getOffer(
    id: string,
    callback: (err: Error, offer: TradeOfferManager.TradeOffer) => any
  );
...
declare namespace TradeOfferManager {
  ...
  class TradeOffer {
    ...
    getUserDetails(
      callback: (err: Error, me: IDetails, them: IDetails) => void
    );
    ...
    accept(
      autoRetry: boolean,
      callback: (err: Error, status: "pending" | "accepted") => void
    );
    ...
    getReceivedItems(
      getActions: boolean,
      callback: (err: Error, items: EconItem[]) => void
    );
